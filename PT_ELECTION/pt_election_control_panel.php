...
$sql_check = pg_query($db, "select * from pt_election_admin where admin_user='$admin_user' and admin_pass='$admin_pass';") or die ("Could not match data because ".pg_last_error());
...
$row = pg_fetch_assoc($sql_check);
 ...
 if ($num==1 ){ 
    ...
    $_SESSION['pt_user'] = $row['admin_user']; 
    $_SESSION['pt_pass'] = $row['admin_pass']; 

    //echo "<script>alert('login failed!');</script>";

    //header('Location: election_admin.php');
    echo "<script>window.location.replace('pt_election_control_panel_table.php')</script>";
  ...
  <br>
    ...
    <div class="row">
      ...
      <div class="col-12 text-center">
            ...
            <button type="submit" class="btn btn-primary" id="SUBMIT" name="SUBMIT_USER">Select</button>
      </div> 
