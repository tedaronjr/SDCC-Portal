...
SET is_open_voting=true , chairperson_status = TRUE,
vice_chairperson_status=false, 
secretary_status=false, treasurer_status=false, auditor_status=false
WHERE organization_id='$array[1]';          

") or die ("Could not match data because ".pg_last_error()); 
     
}

elseif (intval($array[0])==2) {

    $sql_ = pg_query($db, "
    
    UPDATE organizations_voting
    SET is_open_voting=true , chairperson_status = false,
    vice_chairperson_status=true, 
    secretary_status=false, treasurer_status=false, auditor_status=false
    WHERE organization_id='$array[1]';          
...
elseif (intval($array[0])==3) {
        ...
        SET is_open_voting=true , chairperson_status = false,
        vice_chairperson_status=false, 
        secretary_status=true, treasurer_status=false, auditor_status=false
        WHERE organization_id='$array[1]';          
...
elseif (intval($array[0])==4) {
    ...
    SET is_open_voting=true , chairperson_status = false,
    vice_chairperson_status=false, 
    secretary_status=false, treasurer_status=true, auditor_status=false
    WHERE organization_id='$array[1]';          
    
    ") or die ("Could not match data because ".pg_last_error()); 
         
}  
elseif (intval($array[0])==5) {
    ...
    SET is_open_voting=true , chairperson_status = false,
    vice_chairperson_status=false, 
    secretary_status=false, treasurer_status=false, auditor_status=true
    WHERE organization_id='$array[1]';          
    
    ") or die ("Could not match data because ".pg_last_error()); 
         
} 
elseif (intval($array[0])==0) {

    $sql_ = pg_query($db, "
    
    UPDATE organizations_voting
    SET is_open_voting=false , chairperson_status = false,
    vice_chairperson_status=false, 
    secretary_status=false, treasurer_status=false, auditor_status=false
    WHERE organization_id='$array[1]';          
