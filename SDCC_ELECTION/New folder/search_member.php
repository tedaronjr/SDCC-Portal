...
elseif (isset($_POST['Add_Voter'])) {
	...
	$sql_add = pg_query($db, "INSERT INTO election_user(
	 ...
	 select '$fullname','$mem_no','$sc_acctno','$ou_code','$passcode','$pt_no','$updated_at','$managed_by';") or die ("Could not match data because ".pg_last_error());
	$num=0;	
}
...
<form autocomplete="off" role="form"  id="myForm" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
	<div class="row">
		...
		<div class="col-2">
		 <input type="submit" class="btn btn-primary"  value="Search by Account No." id="submit_acctno" name="submit_acctno">
		</div>
		<div class="col-3">
		</div>				
	</div>
...
<form autocomplete="off" role="form"  id="myForm1" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
	<div class="row">
		...
		<div class="col-2">
		 <input type="submit" class="btn btn-primary"  value="Search by Last Name" id="submit_lastname" name="submit_lastname">
		</div>
		<div class="col-3">
		</div>				
	</div>
...
elseif ($num_sql==1)
echo '<td>'."<form  onsubmit='return confirm(\"Are you sure?\");' method=POST target=_self ><input type='submit'  class='btn btn-danger'  name='Del_Voter' value='Delete Voter' />".'</td>';
else
